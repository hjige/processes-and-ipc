...
/// @brief Terminates the running thread, setting any waiting threads to ready.
void manage_threads() {
  PAUSE_TIMER();
...
/// @param ms time in miliseconds for the timer. 
void set_timer(int type, void (*handler) (int), int ms) {
  ...
  // Configure the timer to trigger signal after ms msec
  timer.it_value.tv_sec = 0;
  timer.it_value.tv_usec = ms*1000;
  // Configure the timer to restart after trigger
  timer.it_interval.tv_sec = 0;
  timer.it_interval.tv_usec = ms*1000;

  // Initiate global time values
...
/// @brief Signal handler for SIGALRM, generated by process timer.
///        Rotates running thread into ready queue,
///        resuming head of ready queue once the thread's timeslice is exceeded.
/// @param signum signal recieved.
void timer_context_switch(int signum) {
  printf("PRE-EMPTION: timeslice expired!\n");
  yield();
}
...
int init(int timeslice_in_ms){
  ...
  // Set main process as the currently running thread.
  main_thread->state = running;
  running_thread = main_thread;

  // Start timer for time slice pre-emption
  if (timeslice_in_ms > 0) {
    set_timer(TIMER_TYPE, timer_context_switch, timeslice_in_ms);
  }
...
tid_t join(tid_t thread_id) {
  ...
  // Move running thread to waiting queue.
